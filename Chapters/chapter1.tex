...
% TODO: Antes deste paragrafo seria interessantes incluir aqui um paragrafo sobre ataques que são possiveis de fazer. Nomeadamente, uns em especifico, os Website fingerprinting attacks. O tor é também vulneravel a estes ataques e permitem que o atacante percebe awquilo que esta a ser visto so olhadno para o trafego. Existem exemplos destes ataques no artigo que disse para dares uma vista de olhos no discord.

% ? Queres dizer que antes descrevo os ataques e técnicas utilizadas por regimes autoritários para oprimir e bloquear o acesso a conteudos, meto esta frase a introduzir as anonymity networks que servem para evitar algumas destas vulnerabilidades e depois descrevo que estas networks tambem são vitimas de ataques e descrevo?
...
%TODO: Aqui deves ter as soluçoes segmentadas pelos tipos. Existem varios tipos de soluçoes de evasao a censura, desde encapsulamento de trafego, fully encrypted protocols, traffic shapping etc. Deverias colocar referencias para cada (olhar aqui para os related works dos artigos do discord) e explicar por grupos. Por exemplo: "existem diversas soluçoes, aqui discutimos algumas como traffic encapsulation, etc etc. Depois explicar o que e traffic encapsulation e dos restantes tipos e so depois falar de exemplos concretos.
...
%%% Traffic Shapping
Traffic Shaping is a technique that bla bla bla

Researchers have proposed various solutions.~\citeauthor{kAnonymityEffectiveness} demonstrated that K-Anonymity is effective against traffic analysis attacks, such as Statistical Disclosure and long-term intersection attacks.~\cite{kAnonymityEffectiveness}~\citeauthor{nunes2023kfunnels} proposed BriK, a Tor pluggable transport, that ensures uniform traffic patterns to make users indistinguishable.~\cite{nunes2023kfunnels} Some work as Walkie-Talkie~\cite{WalkieTalkie}, Supersequence~\cite{Supersequence} and Glove~\cite{Glove} also use traffic shaping for defending against website fingerprinting attacks.~\citeauthor{TrafficMorphing} proposed Traffic Morphing~\cite{TrafficMorphing}, a traffic shaping technique that uses a Markov Chain to generate traffic patterns that are indistinguishable from the original traffic.
%%% Others

%% Intro to especific & existing solutions
...
% TODO: Isto entra aqui de forma um pouco intensa. Se utilizares traffic shapping para explicar, como disse na nota do paragrafo anteriro, podes dizer que existem algumas soluçoes que usam privacidade diferencial (citar o artigo que disse no disc) e depois sim falar de privacidade diferencial, o porque de ser fixe, e como nunca foi usada para o tor.
...
% TODO: nao sei se e a eficacia ou se é mais o future proof. ou seja, no fundo sem provas das garantias de segurança nao podes dizer que a tua soluçao e resistente a ataques futuros, so aqueles que tu testaste.
...
% TODO: ^ Falar aqui mais de website fingerprinting. ^
